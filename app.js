...
async function ativarAba(idAba) {
  ...
  // ----------------------------------------------------------
  // A aba agora é responsável por ler o CSV.
  // Isso garante que cada clique utilize os dados atuais.
  // ----------------------------------------------------------
...
async function atualizarTodosOsAtivos() {
  ...
  for (
    ...
    try {
      ...
      // Nenhuma atribuição ocorre antes da validação,
      // logo as colunas 6–11 permanecem intactas neste caso.
  ...
  patrimonio =
    calcularColunasPatrimonio(patrimonio);
  ...
  registrarProgresso(
    "Recarregando patrimônio gravado..."
  );
  ...
  try {
    ...
    registrarProgresso(
      "Tabela recarregada com os dados gravados."
    );

  } catch (erro) {

    console.error(
      "Erro ao recarregar CSV gravado:",
      erro
    );
...
async function gravarPatrimonioNoWorker() {
  ...
  try {
    ...
    throw new Error(
      `Worker de CSV retornou uma resposta inválida ` +
