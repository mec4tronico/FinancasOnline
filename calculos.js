...
export function calcularColunasPatrimonio(patrimonio) {
    ...
    const linhasCalculadas = patrimonio.map(registro => {
        ...
        const min52 = converterNumero(registro.Min52, "Min52", ativo);
        const max52 = converterNumero(registro.Max52, "Max52", ativo);
        const dy = converterNumero(registro.DY, "DY", ativo);

        const valorAtualPosicao = quantidade * valorAtual;
        const lucroPrejuizo = valorAtualPosicao - totalInvestido;
        const rendaAnualEstimada = valorAtualPosicao * dy / 100;
        const valorPosicaoMax52 = quantidade * max52;
        const valorPosicaoMin52 = quantidade * min52;
        ...
        return {
            ...
            rendaAnualEstimada,
            rendaMensalEstimada: rendaAnualEstimada / 12,
            valorPosicaoMax52,
            valorPosicaoMin52,
    ...
    const valorTotalDaCarteira = linhasCalculadas.reduce(
        (total, linha) => total + linha.valorAtualPosicao,
        0
    );
    ...
    return linhasCalculadas.map(linha => ({
        ...linha.registro,
        ValorAtualPosicao: formatarResultado(linha.valorAtualPosicao),
        LucroPrejuizo: formatarResultado(linha.lucroPrejuizo),
        Rentabilidade: linha.rentabilidade === null
            ? ""
            : formatarResultado(linha.rentabilidade),
        PesoCarteira: formatarResultado(
            linha.valorAtualPosicao / valorTotalDaCarteira * 100
        ),
        RendaAnualEstimada: formatarResultado(linha.rendaAnualEstimada),
        RendaMensalEstimada: formatarResultado(linha.rendaMensalEstimada),
        ValorPosicaoMax52: formatarResultado(linha.valorPosicaoMax52),
        ValorPosicaoMin52: formatarResultado(linha.valorPosicaoMin52),
        PotencialFinanceiroMax52: formatarResultado(
            linha.potencialFinanceiroMax52
        ),
        RiscoFinanceiroMin52: formatarResultado(
